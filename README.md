...
This model can be entirely reproduced using the code within /scripts. The .js GEE script, given a 
drainage basin geometry (can be easily pulled from https://streamstats.usgs.gov/ss/?gage=09260050&tab=info)
will export a CSV containing all necessary meteorological and snowpack data for the model. Other 
...
to test, those can be edited by simply changing the water year specified. No changes to the downloaded 
.csv's are necessary and will likely do more harm than good. The R program is designed to run with the 
files exactly as downloaded from GEE and USGS.
...
For each year reported, the program will calculate: 
  ...
  The graph will display like this:
...
years, this model performs excellently with KGE and NSE close to or above 0.9, and PBIAS within 
